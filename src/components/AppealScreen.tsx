                  ...
                  <div className="flex-1">
                    ...
                    {conditions.length > 0 ? (
                      <p className="text-sm text-teal-800 leading-relaxed">
                        <strong>{conditions.map((c: any) => c.name).join(', ')}</strong> — your Appeal will be personalised to these conditions.
                      </p>
                    ) : (
                      <p className="text-sm text-amber-800 leading-relaxed">
                        Add your conditions so PIPpal can write a personalised appeal. Without this your case will be generic.
                      </p>
                    )}
                  </div>
          ...
          {!hasAnswers ? (
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button type="button" onClick={copyReasons}
                ...
                <button type="button"
                  onClick={() => {
                    if (navigator.share) {
                      navigator.share({ title: 'PIPpal — SSCS1 Appeal Reasons', text: appealReasons ?? '' }).catch(() => {});
                    } else { copyReasons(); }
                  }}
                ...
                <button type="button"
                  onClick={async () => {
                    ...
                    const text = (appealReasons ?? '');
                    const lines = text.split('\n');
                    const docChildren = [new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({ text: 'PIPpal SSCS1 Appeal Reasons', bold: true, size: 32, font: 'Arial' })] }), ...lines.map(line => line.trim() === '' ? new Paragraph('') : new Paragraph({ children: [new TextRun({ text: line, size: 24, font: 'Arial' })] }))];
                    const doc = new Document({ sections: [{ children: docChildren }] });
                    const buf = await Packer.toBlob(doc);
                    const url = URL.createObjectURL(buf);
                    const a = document.createElement('a');
                    a.href = url; a.download = 'PIPpal-SSCS1-Appeal.docx'; a.click();
