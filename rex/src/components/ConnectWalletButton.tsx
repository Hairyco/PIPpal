...
export function ConnectWalletButton({ className = '' }: { className?: string }) {
  ...
  if (address) {
    return (
      <button
        ...
        className={`inline-flex h-10 items-center gap-2 rounded-lg border border-[#c8ff3d]/30 bg-[#c8ff3d]/10 px-3 text-xs font-semibold text-[#d5ff69] transition hover:bg-[#c8ff3d]/15 disabled:opacity-50 ${className}`}
      >
  ...
  return (
    <button
      ...
      disabled={busy}
      className={`inline-flex h-10 items-center gap-2 rounded-lg border border-white/[0.1] bg-white/[0.04] px-3 text-xs font-semibold text-white transition hover:border-[#c8ff3d]/35 hover:bg-[#c8ff3d]/10 hover:text-[#d5ff69] disabled:opacity-50 ${className}`}
    >
      <Wallet className="h-3.5 w-3.5 shrink-0" />
