export default async function handler(req, res) {
  ...
  const PIP_KEYWORDS = ['pip', 'personal independence payment', 'disability benefit', 'dwp', 'pip claim', 'pip assessment', 'pip award', 'disabled'];
  const TAG_RULES = [
  ...
  async function fetchFeed(source) {
    try {
      ...
      const resp = await fetch(source.url, {
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; RSS/1.0)', 'Accept': 'application/rss+xml, application/atom+xml, */*' },
  ...
  try {
    ...
    if (allItems.length === 0) {
      return res.status(200).json({ articles: [], message: 'No PIP articles found' });
    }
